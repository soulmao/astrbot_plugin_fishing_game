...
*{box-sizing:border-box}html,body{margin:0;padding:0;background:transparent}body{width:1200px;padding:30px;font-family:"Microsoft YaHei","Noto Sans CJK SC",sans-serif;color:#17212b;background:radial-gradient(circle at 86% 8%,rgba(123,211,255,.32),transparent 31%),linear-gradient(145deg,#d9f3ff 0%,#eefaff 48%,#fff 100%)}
.sheet{overflow:hidden;border:1px solid #a8d9ef;border-radius:24px;background:rgba(255,255,255,.96);box-shadow:0 18px 48px rgba(46,122,158,.18)}.header{display:flex;justify-content:space-between;align-items:flex-end;gap:24px;padding:27px 34px 24px;border-bottom:1px solid #c7e7f6;background:linear-gradient(100deg,#caedff 0%,#eaf8ff 55%,#fff 100%)}
.title{font-size:34px;font-weight:900;color:#102a38}.subtitle{margin-top:7px;color:#54798b;font-size:15px}.wallet{text-align:right}.wallet-label{color:#668391;font-size:13px}.wallet-value{margin-top:3px;color:#9a6600;font-size:25px;font-weight:900}.body{padding:28px 34px 34px}.shop-meta{display:flex;flex-wrap:wrap;gap:9px;margin-bottom:20px}.meta{padding:8px 12px;border-radius:10px;background:#edf8fd;color:#285263;font-weight:700}
.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.card{min-width:0;padding:16px;border:1px solid #c9e4f1;border-radius:16px;background:linear-gradient(145deg,#f5fbfe,#fff);box-shadow:0 6px 16px rgba(41,128,169,.07)}.card.unaffordable{background:#fbfcfd;opacity:.72}.card-head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px}.index{display:inline-flex;align-items:center;justify-content:center;min-width:32px;height:32px;border-radius:10px;background:#dff2fc;color:#126c9e;font-weight:900}.type{padding:4px 8px;border-radius:999px;background:#eef7fb;color:#577482;font-size:12px;font-weight:800}.name{margin-top:12px;font-size:18px;line-height:1.35;font-weight:900;color:#153746}.quantity{margin-left:5px;color:#607d8b;font-size:14px}.skills{display:flex;flex-wrap:wrap;gap:6px;margin-top:11px}.skill{padding:5px 8px;border:1px solid #c6e2ef;border-radius:8px;background:#f7fcfe;color:#24566d;font-size:12px;font-weight:700}.skill b{margin-left:3px;color:#1374b7}.price-row{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:14px;padding-top:12px;border-top:1px solid #e0eef4}.price{color:#a66d00;font-size:18px;font-weight:900}.state{color:#7f929b;font-size:12px;font-weight:700}.empty{grid-column:1/-1;padding:48px;text-align:center;border:1px dashed #bddce9;border-radius:16px;color:#78909c;background:#f8fcfe}.hint{margin-top:22px;padding:12px 14px;border-radius:12px;background:#edf8fd;color:#496979;text-align:center;font-size:14px}
</style></head><body><article class="sheet"><header class="header"><div><div class="title">🏪 钓鱼商店</div><div class="subtitle">{{ user_name }} 的专属货架</div></div><div class="wallet"><div class="wallet-label">当前金币</div><div class="wallet-value">💰 {{ coins }}</div></div></header><main class="body"><div class="shop-meta"><span class="meta">商店 Lv.{{ shop_level }}</span><span class="meta">展示位 {{ items|length }} / {{ slot_count }}</span><span class="meta">刷新 {{ refresh_status }}</span></div><section class="grid">{% for item in items %}<article class="card{% if not item.affordable %} unaffordable{% endif %}"><div class="card-head"><span class="index">{{ item.index }}</span><span class="type">{{ item.icon }} {{ item.type_name }}</span></div><div class="name">{{ item.name }}{% if item.quantity %}<span class="quantity">× {{ item.quantity }}</span>{% endif %}</div>{% if item.skills %}<div class="skills">{% for skill in item.skills %}<span class="skill">{{ skill.name }} <b>{{ skill.value }}</b></span>{% endfor %}</div>{% endif %}<div class="price-row"><span class="price">{{ item.price }} 金币</span><span class="state">{% if item.affordable %}可购买{% else %}金币不足{% endif %}</span></div></article>{% else %}<div class="empty">商店正在补货，请稍后刷新</div>{% endfor %}</section><div class="hint">使用 /购买 编号 数量　·　使用 /刷新商店 更换商品</div></main></article></body></html>
"""
...
<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><style>
*{box-sizing:border-box}html,body{margin:0;padding:0;background:transparent}body{width:1200px;padding:30px;font-family:"Microsoft YaHei","Noto Sans CJK SC",sans-serif;color:#17212b;background:radial-gradient(circle at 86% 8%,rgba(123,211,255,.32),transparent 31%),linear-gradient(145deg,#d9f3ff 0%,#eefaff 48%,#fff 100%)}
.sheet{overflow:hidden;border:1px solid #a8d9ef;border-radius:24px;background:rgba(255,255,255,.96);box-shadow:0 18px 48px rgba(46,122,158,.18)}.header{display:flex;justify-content:space-between;align-items:flex-end;gap:24px;padding:27px 34px 24px;border-bottom:1px solid #c7e7f6;background:linear-gradient(100deg,#caedff 0%,#eaf8ff 55%,#fff 100%)}.title{font-size:34px;font-weight:900;color:#102a38}.subtitle{margin-top:7px;color:#54798b;font-size:15px}.page{text-align:right;color:#176b98;font-size:17px;font-weight:900}.body{padding:28px 34px 34px}.search{display:inline-flex;margin-bottom:18px;padding:8px 12px;border-radius:10px;background:#edf8fd;color:#285263;font-weight:700}
.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.card{min-width:0;padding:17px;border:1px solid #c9e4f1;border-radius:16px;background:linear-gradient(145deg,#f5fbfe,#fff);box-shadow:0 6px 16px rgba(41,128,169,.07)}.card.own{border:2px solid #55a6df;background:linear-gradient(145deg,#e7f6ff,#fff)}.card-head{display:flex;align-items:center;justify-content:space-between;gap:10px}.type{padding:5px 9px;border-radius:999px;background:#e8f5fb;color:#2c657d;font-size:12px;font-weight:800}.own-badge{margin-left:6px;color:#1475b8}.enhance{padding:3px 9px;border-radius:999px;background:#1676c2;color:#fff;font-weight:900}.name{margin-top:12px;font-size:19px;line-height:1.35;font-weight:900;color:#153746}.quantity{margin-left:5px;color:#607d8b;font-size:14px}.skills{display:flex;flex-wrap:wrap;gap:6px;margin-top:10px}.skill{padding:5px 8px;border:1px solid #c6e2ef;border-radius:8px;background:#f7fcfe;color:#24566d;font-size:12px;font-weight:700}.skill b{margin-left:3px;color:#1374b7}.trade{display:grid;grid-template-columns:1.1fr 1fr 1fr;gap:8px;margin-top:14px;padding-top:12px;border-top:1px solid #e0eef4}.label{display:block;color:#78909c;font-size:11px}.value{display:block;margin-top:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#314f5d;font-size:13px;font-weight:800}.price{color:#a66d00;font-size:17px}.listing-id{margin-top:11px;padding:7px 9px;border-radius:8px;background:#f0f7fa;color:#526f7d;font-family:Consolas,monospace;font-size:12px;word-break:break-all}.rarity-rare{color:#1478c9}.rarity-legendary{color:#d52f45}.rarity-mythic{color:#8438b5}.ancient{color:#a76d00}.empty{grid-column:1/-1;padding:58px;text-align:center;border:1px dashed #bddce9;border-radius:16px;color:#78909c;background:#f8fcfe}.hint{margin-top:22px;padding:12px 14px;border-radius:12px;background:#edf8fd;color:#496979;text-align:center;font-size:14px}
