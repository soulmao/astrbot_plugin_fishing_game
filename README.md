...
| 🤖 **LLM 集成** | 19 个 FunctionTool，支持自然语言多步工具调用 |
...
插件注册了 20 个 **FunctionTool**，支持通过自然语言与钓鱼游戏交互，且支持多步工具调用。
...
├── llm_tools.py             # 20 个 LLM FunctionTool 定义
...
| `llm_tools.py` | ~512 | 20 个 FunctionTool 的 dataclass 定义 |
...
│  - LLM FunctionTool 注册 (20 个工具)                         │
│  - 定时任务 (每日刷新、拍卖行过期检查)                        │
│  - 贪婪打乱效果 _apply_greedy_scramble()                     │
└──────────────────────┬──────────────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────────────┐
│  Command Layer (command_*.py × 8)                            │
│  ┌──────────────┐ ┌──────────────┐ ┌─────────────────────┐ │
...
│  └──────────────┘ └──────────────┘ └─────────────────────┘ │
│  ┌──────────────┐ ┌──────────────┐                          │
│  │ EnchantCmds  │ │ Achievement  │  均继承自 CommandBase    │
│  │ (附魔/升级)  │ │ (成就系统)   │  (user_locks / _get_user_lock)│
│  └──────────────┘ └──────────────┘                          │
└──────────────────────┬──────────────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────────────┐
