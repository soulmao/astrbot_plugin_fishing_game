...
| 🤖 **LLM 集成** | 25 个 FunctionTool，支持自然语言多步工具调用 |
...
钓鱼、继续贪婪和收杆使用专用结算模板：展示本次渔获、稀有度、总价值、经验、冷却、技能事件，以及贪欲结晶层数和断线风险。普通失败、资源不足和冷却状态也会进入统一的垂钓状态页。
...
├── llm_tools.py             # 25 个 LLM FunctionTool 定义
...
├── fishing_renderer.py      # 钓鱼、贪婪挑战与收杆结算模板
...
25 个 FunctionTool 基于 **Pydantic dataclass** 定义，每个工具包含：
- `name`：全局唯一的函数名
