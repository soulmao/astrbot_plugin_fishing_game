...
| 前缀 | 图标 | 机制 |
|------|------|------|
| 贪婪的 | 💰 | 必定触发 2~5 倍钓鱼，高稀有度权重，额外金币馈赠；每轮消耗 2 个鱼饵，冷却延长 10% |
| 无尽贪婪的 | ♾️ | 贪婪升级版，3~7 倍钓鱼，稀有度权重更高，钓到的鱼售价 +20%；每轮消耗 3 个鱼饵，冷却延长 25%，20% 概率反噬导致无渔获。可通过 `/切换贪婪` 与旧贪婪互相切换 |
| 诅咒的 | 👻 | 附魔和升级异常便宜，但钓竿不稳定，可能丢失词条 |
| 迅捷的 | ⚡ | 冷却极快，偶尔钓鱼失败 |
| 学徒的 | 📚 | 大量经验加成，但金币收益减少 |
| 幸运方块的 | 🎲 | 随机添加或消除技能 |
| 傲慢的 | 👑 | 仅限金色/神级钓竿。睥睨：无视常见/稀有鱼；自负：必须使用珍稀（香料饵）及以上鱼饵，否则拒绝工作 |
| 嫉妒的 | 💢 | 攀比之力：每有一名等级更高的玩家，稀有度权重 +5%（最高 +50%）；钓到传说/神话鱼时可能额外消耗金币或经验 |
