...
*{box-sizing:border-box}html,body{margin:0;padding:0;background:transparent}body{width:1200px;padding:30px;font-family:"Microsoft YaHei","Noto Sans CJK SC",sans-serif;color:#17212b;background:radial-gradient(circle at 86% 8%,rgba(123,211,255,.32),transparent 31%),linear-gradient(145deg,#d9f3ff 0%,#eefaff 48%,#fff 100%)}.sheet{overflow:hidden;border:1px solid #a8d9ef;border-radius:24px;background:rgba(255,255,255,.96);box-shadow:0 18px 48px rgba(46,122,158,.18)}.header{display:flex;justify-content:space-between;align-items:flex-end;gap:24px;padding:27px 34px 24px;border-bottom:1px solid #c7e7f6;background:linear-gradient(100deg,#caedff 0%,#eaf8ff 55%,#fff 100%)}.title{font-size:34px;font-weight:900;color:#102a38}.subtitle{margin-top:7px;color:#54798b;font-size:15px}.summary{text-align:right;color:#176b98;font-size:16px;font-weight:900}.body{padding:28px 34px 34px}.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.card{min-width:0;padding:16px;border:1px solid #c9e4f1;border-radius:16px;background:linear-gradient(145deg,#f5fbfe,#fff);box-shadow:0 6px 16px rgba(41,128,169,.07)}.card.current{border:2px solid #258be3;background:linear-gradient(145deg,#e2f5ff,#fff);box-shadow:0 0 0 3px rgba(37,139,227,.10)}.head{display:flex;align-items:center;justify-content:space-between;gap:10px}.index{display:inline-flex;align-items:center;justify-content:center;min-width:32px;height:32px;border-radius:10px;background:#dff2fc;color:#126c9e;font-weight:900}.badges{display:flex;gap:6px}.count{padding:4px 9px;border-radius:999px;background:#eef7fb;color:#436878;font-size:12px;font-weight:800}.current-badge{padding:4px 9px;border-radius:999px;background:#258be3;color:#fff;font-size:12px;font-weight:800}.name{margin-top:12px;font-size:18px;font-weight:900;color:#153746}.effects{display:flex;flex-wrap:wrap;gap:7px;margin-top:12px}.effect{padding:6px 9px;border:1px solid #c6e2ef;border-radius:9px;background:#f8fcfe;color:#456674;font-size:12px;font-weight:700}.effect b{margin-left:4px;color:#1374b7;font-size:14px}.empty{grid-column:1/-1;padding:58px;text-align:center;border:1px dashed #bddce9;border-radius:16px;color:#78909c;background:#f8fcfe}.hint{margin-top:22px;padding:12px 14px;border-radius:12px;background:#edf8fd;color:#496979;text-align:center;font-size:14px}
...
<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><style>
*{box-sizing:border-box}html,body{margin:0;padding:0;background:transparent}body{width:1200px;padding:30px;font-family:"Microsoft YaHei","Noto Sans CJK SC",sans-serif;color:#17212b;background:radial-gradient(circle at 86% 8%,rgba(123,211,255,.32),transparent 31%),linear-gradient(145deg,#d9f3ff 0%,#eefaff 48%,#fff 100%)}.sheet{overflow:hidden;border:1px solid #a8d9ef;border-radius:24px;background:rgba(255,255,255,.96);box-shadow:0 18px 48px rgba(46,122,158,.18)}.header{padding:27px 34px 24px;border-bottom:1px solid #c7e7f6;background:linear-gradient(100deg,#caedff 0%,#eaf8ff 55%,#fff 100%)}.title{font-size:34px;font-weight:900;color:#102a38}.subtitle{margin-top:7px;color:#54798b;font-size:15px}.body{padding:28px 34px 34px}.overall{padding:17px 19px;border:1px solid #c8e6f4;border-radius:15px;background:#f4fbff}.overall-head{display:flex;justify-content:space-between;gap:20px;margin-bottom:10px}.overall-value{color:#12658b;font-size:18px;font-weight:900}.overall-pct{color:#527487;font-weight:800}.progress{height:13px;overflow:hidden;border-radius:999px;background:#dceef7}.fill{height:100%;min-width:4px;border-radius:inherit;background:linear-gradient(90deg,#4bb7e8,#2387df)}.rarities{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-top:18px}.rarity{padding:15px;border:1px solid #d2e8f2;border-radius:14px;background:#fbfdfe}.rarity-name{font-weight:900}.rarity-count{margin-top:7px;font-size:20px;font-weight:900}.rarity-pct{margin-top:3px;color:#718995;font-size:12px}.common{color:#26343b}.rare{color:#1478c9}.legendary{color:#d52f45}.mythic{color:#8438b5}.section-title{margin:26px 0 12px;font-size:20px;font-weight:900;color:#153746}.recent{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.item{min-width:0;padding:12px 14px;border:1px solid #d5e8f1;border-radius:12px;background:#fbfdfe}.fish-name{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:15px;font-weight:900}.fish-meta{display:flex;justify-content:space-between;gap:8px;margin-top:7px;color:#748b96;font-size:12px}.ancient{color:#a76d00}.empty{grid-column:1/-1;padding:42px;text-align:center;border:1px dashed #bddce9;border-radius:14px;color:#78909c;background:#f8fcfe}
...
<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><style>
*{box-sizing:border-box}html,body{margin:0;padding:0;background:transparent}body{width:1560px;padding:30px;font-family:"Microsoft YaHei","Noto Sans CJK SC",sans-serif;color:#17212b;background:radial-gradient(circle at 86% 8%,rgba(123,211,255,.32),transparent 31%),linear-gradient(145deg,#d9f3ff 0%,#eefaff 48%,#fff 100%)}.sheet{overflow:hidden;border:1px solid #a8d9ef;border-radius:24px;background:rgba(255,255,255,.96);box-shadow:0 18px 48px rgba(46,122,158,.18)}.header{display:flex;justify-content:space-between;align-items:flex-end;gap:24px;padding:27px 34px 24px;border-bottom:1px solid #c7e7f6;background:linear-gradient(100deg,#caedff 0%,#eaf8ff 55%,#fff 100%)}.title{font-size:34px;font-weight:900;color:#102a38}.subtitle{margin-top:7px;color:#54798b;font-size:15px}.summary{text-align:right}.summary strong{display:block;color:#176b98;font-size:23px}.summary span{color:#607d8b;font-size:13px}.body{padding:25px 34px 34px}.overall{height:12px;overflow:hidden;border-radius:999px;background:#dceef7}.overall-fill{height:100%;min-width:4px;border-radius:inherit;background:linear-gradient(90deg,#4bb7e8,#2387df)}.group{margin-top:23px}.group-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px}.group-title{font-size:19px;font-weight:900;color:#153746}.group-count{color:#698591;font-size:13px;font-weight:800}.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.card{min-width:0;padding:13px 14px;border:1px solid #d5e8f1;border-radius:13px;background:#fbfdfe}.card.done{border-color:#b8dfcf;background:linear-gradient(145deg,#effbf6,#fff)}.card-head{display:flex;align-items:center;justify-content:space-between;gap:8px}.name{font-size:16px;font-weight:900;color:#183b4b}.done .name{color:#14734e}.status{font-size:13px;font-weight:900;color:#78909c}.done .status{color:#18875c}.desc{height:34px;margin-top:6px;color:#647f8c;font-size:12px;line-height:1.4}.progress-row{display:flex;align-items:center;gap:8px;margin-top:9px}.mini-progress{flex:1;height:7px;overflow:hidden;border-radius:99px;background:#e1edf2}.mini-fill{height:100%;min-width:3px;border-radius:inherit;background:#4ba8dc}.done .mini-fill{background:#45b889}.numbers{color:#5f7b88;font-size:11px;font-weight:800}.reward{margin-top:8px;color:#886400;font-size:11px;font-weight:800}
</style></head><body><article class="sheet"><header class="header"><div><div class="title">🏅 成就系统</div><div class="subtitle">{{ user_name }} 的荣誉陈列</div></div><div class="summary"><strong>{{ completed }} / {{ total }}</strong><span>总完成度 {{ percent }}%</span></div></header><main class="body"><div class="overall"><div class="overall-fill" style="width:{{ percent }}%"></div></div>{% for group in groups %}<section class="group"><div class="group-head"><div class="group-title">{{ group.icon }} {{ group.name }}</div><div class="group-count">{{ group.completed }} / {{ group.total }} 已完成</div></div><div class="grid">{% for achievement in group.items %}<article class="card{% if achievement.done %} done{% endif %}"><div class="card-head"><span class="name">{{ achievement.name }}</span><span class="status">{% if achievement.done %}已解锁{% else %}进行中{% endif %}</span></div><div class="desc">{{ achievement.description }}</div><div class="progress-row"><div class="mini-progress"><div class="mini-fill" style="width:{{ achievement.percent }}%"></div></div><span class="numbers">{{ achievement.current }} / {{ achievement.target }}</span></div><div class="reward">奖励 💰{{ achievement.reward_coins }}　📈{{ achievement.reward_exp }}</div></article>{% endfor %}</div></section>{% endfor %}</main></article></body></html>
"""
