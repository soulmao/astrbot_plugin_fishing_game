...
from .fish_data import (
    ...
    get_rod_prefix, get_bait_prefix, scramble_text,
)
...
📖 `/图鉴` - 查看已收集的鱼类图鉴进度"""
    ...
    def _do_fish_once(self, user, rod, selected_bait=None) -> Optional[dict]:
    ...
    async def cmd_fish(self, event) -> str:
        ...
        async with self._get_user_lock(user_id):
            ...
            # 特种钓竿固定技能（覆盖前缀，不受前缀影响）
            if is_gold_rod:
                skills["treasure"] = 0.80  # 金币钓竿固定80%寻宝
            if is_carrot_rod:
                skills["voyage"] = 0.80    # 胡萝卜钓竿固定80%远航
            ...
            if lucky_block_val > 0:
                if random.random() < 0.5:
                    ...
                    available_skills = [s for s in self._get_available_skills() if s not in current_skills and s not in rod_prefix.get("skills", {})]
                    if available_skills:
...
🎣 拥有的钓竿:"""
    ...
    async def cmd_sell(self, event, fish_id_or_all: str = "all") -> str:
        ...
        async with self._get_user_lock(user_id):
            ...
            # 诅咒前缀：卖出鱼竿扣除30%金币
            rod = user.current_rod
            rod_prefix = self._get_rod_prefix(rod.get("prefix_id", ""))
            cursed_penalty = 0
            if rod_prefix.get("skills", {}).get("cursed"):
                cursed_penalty = int(total_earned * 0.30)
                total_earned -= cursed_penalty
            ...
            curse_msg = ""
            if cursed_penalty > 0:
                curse_msg = f"\n👻 诅咒扣除: {cursed_penalty} 金币"
            
            result = f"""💰 出售成功！

获得: {total_earned} 金币 (+{exp_gained} 经验){curse_msg}"""
        ...
        if action in ("出售", "sell"):
            ...
            async with self._get_user_lock(user_id):
                ...
                if item_type == "rod":
                    ...
                    return f"✅ 已直接出售 {self._format_rod_name(rod)}，获得 {sell_price} 金币{penalty_msg}" + self._get_latest_rods_summary(user)
