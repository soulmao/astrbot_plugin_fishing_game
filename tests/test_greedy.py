...
from astrbot_plugin_fishing_game.fish_data import (
    get_rod_prefix, calc_rod_value, SPECIAL_PREFIX_BALANCE,
)
...
class GreedyPureLogicTests(unittest.TestCase):
    ...
    def test_multiplier_tables(self):
        cmds = FishingCommands(MockStar(), MockStorage())
        # stack=1 恒为 1.0x
        self.assertEqual(cmds._get_greedy_total_multiplier(1, "normal"), 1.0)
        self.assertEqual(cmds._get_greedy_total_multiplier(1, "endless"), 1.0)
        # stack=2 对应表第一项
        self.assertEqual(cmds._get_greedy_total_multiplier(2, "normal"), 1.25)
        self.assertEqual(cmds._get_greedy_total_multiplier(2, "endless"), 1.5)
        # stack=3
        self.assertEqual(cmds._get_greedy_total_multiplier(3, "normal"), 1.65)
        self.assertEqual(cmds._get_greedy_total_multiplier(3, "endless"), 2.1)

    def test_multiplier_extrapolation(self):
        cmds = FishingCommands(MockStar(), MockStorage())
        # normal 表长度 10，对应 stack 2..11；stack=12 应线性外推
        expected = 8.45 + (8.45 - 7.25)  # last + step
        self.assertEqual(cmds._get_greedy_total_multiplier(12, "normal"), expected)
...
class GreedyCommandFlowTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_continue_break(self):
        ...
        self.assertFalse(user.is_greedy_active())
        self.assertEqual(user.coins, 900)  # 1000 - 10%
    ...
    async def test_continue_success_no_break(self):
        ...
        self.assertTrue(user.is_greedy_active())
        # 初始 total_price=100, 新鱼 price=20, growth_ratio = 1.25/1.0
        # (100+20)*1.25 = 150
        self.assertEqual(user.greedy_state["chip"]["total_price"], 150)
        self.assertEqual(user.greedy_state["stack"], 2)
