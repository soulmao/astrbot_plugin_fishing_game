...
| `plugin.json` | 插件基本信息 |
| `_conf_schema.json` | AstrBot 面板配置 Schema，支持 `fishing_cooldown`、`shop_refresh_cooldown`、拍卖行相关参数等 |
...
- 并发安全：每个用户独立 `asyncio.Lock`，赠送场景按 user_id 排序加锁防止死锁。
...
- **依赖库**:
  ...
  - 标准库：`asyncio`, `time`, `random`, `uuid`, `datetime`, `typing`
...
3. 在 AstrBot 面板配置插件参数（冷却时间、拍卖行参数等）。
...
| `main.py` | 427 | 插件生命周期、命令路由、LLM 工具注册、定时任务 |
...
| `command_economy.py` | 234 | 卖鱼、商店、购买、刷新商店、商店升级 |
| `command_social.py` | 226 | 赠送金币/渔获/鱼饵/钓竿，含事务回滚 |
| `command_auction.py` | 472 | 拍卖行浏览/搜索/上架/取消/购买 |
| `command_enchant.py` | 285 | 随机附魔、技能升级、定向附魔 |
| `command_achievements.py` | 57 | 成就列表与进度查询 |
| `command_info.py` | 404 | 帮助、背包、等级、图鉴、冷却、排行榜 |
...
| `utils.py` | 325 | 格式化、价值计算、商店生成、加权随机等纯工具函数 |
| `llm_tools.py` | 584 | 22 个 FunctionTool 的 dataclass 定义 |
| `fish_data_admin.html` | - | 独立 HTML 数据管理台（静态文件，未在代码中引用） |
| `_conf_schema.json` | 32 | 插件配置 Schema |
...
- 涉及两个用户的操作（赠送）：按 `sorted([sender_id, receiver_id])` 顺序加锁，防止死锁。
- 拍卖行全局列表操作在 `StorageManager` 中使用 `self._auction_lock`。

### 4.6 错误处理

- 业务错误直接返回中文提示字符串。
- 存储异常需要尝试回滚（参考 `command_social.py` 的赠送逻辑）。
...
- 本项目**没有 CI/CD 流程**和 `.gitignore` 文件。
